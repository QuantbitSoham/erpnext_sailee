...
class ProductionPlan(Document):
	...
	@frappe.whitelist()
	def opcost(self):
		doc=frappe.db.get_list('Sales Order',filters={"name":self.sales_order})
		for d in doc:
			doc1=frappe.get_doc('Sales Order',d.name)
			if(self.sales_order==d.name):
					self.append("sales_orders",{
							"sales_order":doc1.name,
							"sales_order_date":doc1.transaction_date,
							"customer":doc1.customer,
							"grand_total":doc1.grand_total
							}
						)
