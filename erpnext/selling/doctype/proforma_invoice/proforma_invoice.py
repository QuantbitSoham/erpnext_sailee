...
class ProformaInvoice(Document):
	def before_save(self):
		...
		if self.grand_total:
			self.in_words_amt = num2words(self.grand_total)
			self.in_words = num2words(self.grand_total)
	...
	def getitems(self):
		# if self.docstatus!=0:
		so=frappe.db.get_list("Sales Order",fields=["name","po_no","po_date","delivery_date","total_qty","total","net_total","grand_total","rounding_adjustment","rounded_total","in_words","advance_paid","terms"],filters={"name":self.sales_order})
		for k in so:
			# frappe.msgprint(str(k.terms))
			self.terms_details=k.terms 
			# frappe.msgprint(str(k.po_no))
			self.po_no=k.po_no
			self.po_date=k.po_date
			self.delivery_date=k.delivery_date
			# self.total_qty=k.total_qty

			self.total=k.total
			self.net_total_amt=k.net_total
			self.grand_total=k.grand_total
			self.rounding_adjustment=k.rounding_adjustment
			self.rounded_total=k.rounded_total 
			self.in_words_amt=k.in_words
			self.advance_paid=k.advance_paid

		doc = frappe.db.get_list("Sales Order", fields=["name"])
		for so in doc:
			if(self.sales_order==so.name):
				doc2=frappe.get_doc("Sales Order",so.name)
				
				for oc in doc2.get("items"):
					self.append("items", {
					...
					"qty" : oc.qty,
					"uom":oc.uom,
					"price_list_rate" : oc.price_list_rate,
					"rate" : oc.rate,
					...
					"base_net_rate":oc.base_net_rate,
					"base_net_amount":oc.base_net_amount,
...
class PerformaInvoice(SellingController):

	@frappe.whitelist()
	def before_submit(self):
		self.set("temp_table", [])
		# frappe.msgprint("hdgasjfkg")
		list_1 = []
		list_1.clear()
	...
	@frappe.whitelist()
	def after_onload_event(self):
		if self.docstatus==1:
			self.set("delete_items", [])
			list_3 = []
			list_3.clear()
	...
	# @frappe.whitelist()
	# def get_list_of_items_onload(self):
	# 	list_1 = []
	# 	list_1.clear()
	# 	for row in self.items:
	# 		list_1.append({
	...
	# 	item_after_save.clear()
	# 	for row in self.delete_items:
	# 			item_after_save.append({
	# 				"item_code": str(row.item_code),
	# 				"qty":str(row.qty),
	# 				"rate":str(row.rate),
	...
	# 	for row in item_after_save:
	# 		self.append("temp_1_table", {
    #             "item_code": row["item_code"],
    #             "qty": row["qty"],
    #             "rate": row["rate"],
	...
	# 	save_temp=[]
	# 	save_temp.clear()
	# 	for i in self.temp_1_table:
	# 		save_temp.append({
	# 			"item_code": str(i.item_code),
	# 			"qty":str(i.qty),
	# 			"rate":str(i.rate)
	# 		})
	# 	# frappe.msgprint(str(save_temp))
	# 	for row in save_temp:
	# 			self.append("delete_items", {
	# 				"item_code": row["item_code"],
	# 				"qty": row["qty"],
	# 				"rate": row["rate"],
	...
	# 			})
		...
		# frappe.msgprint(str(list_3))
		# frappe.msgprint(str(list_2))
		# frappe.msgprint(str(delete_item))
...
# 	@frappe.whitelist()
# 	def get_list_of_items_onload(self):
# 		global dict_1, dict_2, deleteitem
# 		for row in self.items:
# 			dict_key = f"{row.item_code}-{row.qty}"
# 			dict_1[dict_key] = {
# 				"item_code": str(row.item_code),
# 				"qty":str(row.qty),
# 				"rate":str(row.rate)
# 			}
# 		frappe.msgprint(str(dict_1))
# 		for dict_key, values in dict_1.items():
# 			row = self.append('temp_table', {})
# 			row.item_code = values.get('item_code')
# 			row.qty = values.get('qty')
# 			row.rate = values.get('rate')
# 			row.save()
...
# 			}
# 		# frappe.msgprint(str(dict_1))
# 		# frappe.msgprint(str(dict_2))
# 		dict_1.clear()
# 		for i in self.temp_table:
# 			dict_key = f"{i.item_code}-{i.qty}"
# 			dict_1[dict_key] = {
# 				"item_code": str(i.item_code),
# 				"qty":str(i.qty),
# 				"rate":str(i.rate)
...
# 		frappe.msgprint(str(dict_1))
# 		frappe.msgprint(str(dict_2))
# 		frappe.msgprint(str(deleteitem))
	...
	def __init__(self, *args, **kwargs):
		super(PerformaInvoice, self).__init__(*args, **kwargs)

	def validate(self):
		super(PerformaInvoice, self).validate()
	...
	def validate_po(self):
		...
		if self.po_no and self.customer and not self.skip_delivery_note:
			so = frappe.db.sql(
				"select name from `tabPerforma Invoice` \
			...
			if (
				...
				frappe.msgprint(
					_("Warning: Performa Invoice {0} already exists against Customer's Purchase Order {1}").format(
						so[0][0], self.po_no
					)
				)
			...
			if self.delivery_date:
				for d in self.get("items"):
					...
					if getdate(self.transaction_date) > getdate(d.delivery_date):
						frappe.msgprint(
							_("Expected Delivery Date should be after Performa Invoice Date"),
							indicator="orange",
							title=_("Warning"),
						)
			else:
	...
	def validate_warehouse(self):
		super(PerformaInvoice, self).validate_warehouse()
	...
	def validate_with_previous_doc(self):
		super(PerformaInvoice, self).validate_with_previous_doc(
	...
	def on_cancel(self):
		...
		super(PerformaInvoice, self).on_cancel()
	...
	def check_credit_limit(self):
		# if bypass credit limit check is set to true (1) at Performa Invoice level,
	...
	def check_nextdoc_docstatus(self):
		linked_invoices = frappe.db.sql_list(
			...
			where t1.name = t2.parent and t2.performa_invoice = %s and t1.docstatus = 0""",
			self.name,
		)
		...
		if linked_invoices:
			...
			frappe.throw(
				_("Sales Invoice {0} must be deleted before cancelling this Performa Invoice").format(
					", ".join(linked_invoices)
				)
			)

	def check_modified_date(self):
		mod_db = frappe.db.get_value("Performa Invoice", self.name, "modified")
		...
		for item in self.items:
			if item.supplier:
				supplier = frappe.db.get_value(
					"Performa Invoice Item", {"parent": self.name, "item_code": item.item_code}, "supplier"
				)
		...
		for item in self.items:
			if item.delivered_by_supplier:
				item_delivered_qty = frappe.db.sql(
					...
					where poi.performa_invoice_item = %s
						and poi.item_code = %s
			...
			for i in table:
				...
				if not for_raw_material_request:
					total_work_order_qty = flt(
						...
						where production_item=%s and performa_invoice=%s and performa_invoice_item = %s and docstatus<2""",
...
@frappe.whitelist()
def close_or_unclose_performa_invoices(names, status):
	if not frappe.has_permission("Performa Invoice", "write"):
		frappe.throw(_("Not permitted"), frappe.PermissionError)

	names = json.loads(names)
	for name in names:
		so = frappe.get_doc("Performa Invoice", name)
		if so.docstatus == 1:
			if status == "Closed":
...
def get_requested_item_qty(performa_invoice):
	return frappe._dict(
		frappe.db.sql(
			"""
		select performa_invoice_item, sum(qty)
		from `tabMaterial Request Item`
		where docstatus = 1
			and performa_invoice = %s
		group by performa_invoice_item
	""",
			performa_invoice,
		)
	)
...
def make_material_request(source_name, target_doc=None):
	...
	doc = get_mapped_doc(
		...
		source_name,
		{
			"Performa Invoice": {"doctype": "Material Request", "validation": {"docstatus": ["=", 1]}},
			"Packed Item": {
				"doctype": "Material Request Item",
				"field_map": {"parent": "performa_invoice", "uom": "stock_uom"},
				"postprocess": update_item,
			},
			"Performa Invoice Item": {
				"doctype": "Material Request Item",
				"field_map": {"name": "performa_invoice_item", "parent": "performa_invoice"},
...
def make_delivery_note(source_name, target_doc=None, skip_item_mapping=False):
	...
	mapper = {
		"Performa Invoice": {"doctype": "Delivery Note", "validation": {"docstatus": ["=", 1]}},
	...
	if not skip_item_mapping:
		...
		mapper["Performa Invoice Item"] = {
			"doctype": "Delivery Note Item",
			"field_map": {
				"rate": "rate",
				"name": "so_detail",
				"parent": "against_performa_invoice",
			},
			"postprocess": update_item,
			"condition": condition,
		}

	target_doc = get_mapped_doc("Performa Invoice", source_name, mapper, target_doc, set_missing_values)
...
def make_maintenance_schedule(source_name, target_doc=None):
	maint_schedule = frappe.db.sql(
		...
		where t2.parent=t1.name and t2.performa_invoice=%s and t1.docstatus=1""",
		source_name,
	)
	...
	if not maint_schedule:
		doclist = get_mapped_doc(
			...
			source_name,
			{
				"Performa Invoice": {"doctype": "Maintenance Schedule", "validation": {"docstatus": ["=", 1]}},
				"Performa Invoice Item": {
					"doctype": "Maintenance Schedule Item",
					"field_map": {"parent": "performa_invoice"},
				},
			},
			target_doc,
		)
...
def make_maintenance_visit(source_name, target_doc=None):
	...
	if not visit:
		doclist = get_mapped_doc(
			...
			source_name,
			{
				"Performa Invoice": {"doctype": "Maintenance Visit", "validation": {"docstatus": ["=", 1]}},
				"Performa Invoice Item": {
...
def get_events(start, end, filters=None):
	...
	conditions = get_event_conditions("Performa Invoice", filters)

	data = frappe.db.sql(
		"""
		select
			distinct `tabPerforma Invoice`.name, `tabPerforma Invoice`.customer_name, `tabPerforma Invoice`.status,
			`tabPerforma Invoice`.delivery_status, `tabPerforma Invoice`.billing_status,
			`tabPerforma Invoice Item`.delivery_date
		from
			`tabPerforma Invoice`, `tabPerforma Invoice Item`
		where `tabPerforma Invoice`.name = `tabPerforma Invoice Item`.parent
			and `tabPerforma Invoice`.skip_delivery_note = 0
			and (ifnull(`tabPerforma Invoice Item`.delivery_date, '0000-00-00')!= '0000-00-00') \
			and (`tabPerforma Invoice Item`.delivery_date between %(start)s and %(end)s)
			and `tabPerforma Invoice`.docstatus < 2
			{conditions}
		""".format(
...
def make_purchase_order(source_name, selected_items=None, target_doc=None):
	...
	# po = frappe.get_list("Purchase Order", filters={"performa_invoice":source_name, "supplier":supplier, "docstatus": ("<", "2")})
	doc = get_mapped_doc(
		"Performa Invoice",
		source_name,
		{
			...
			"Packed Item": {
				"doctype": "Purchase Order Item",
				"field_map": [
					["name", "performa_invoice_packed_item"],
					["parent", "performa_invoice"],
					["uom", "uom"],
					["conversion_factor", "conversion_factor"],
...
def set_delivery_date(items, performa_invoice):
	delivery_dates = frappe.get_all(
		"Performa Invoice Item", filters={"parent": performa_invoice}, fields=["delivery_date", "item_code"]
	)
...
@frappe.whitelist()
def make_work_orders(items, performa_invoice, company, project=None):
	"""Make Work Orders against the given Performa Invoice for the given `items`"""
	...
	for i in items:
		...
		work_order = frappe.get_doc(
			dict(
				...
				performa_invoice_item=i["performa_invoice_item"],
				project=project,
...
@frappe.whitelist()
def make_raw_material_request(items, company, performa_invoice, project=None):
	if not frappe.has_permission("Performa Invoice", "write"):
		frappe.throw(_("Not permitted"), frappe.PermissionError)
	...
	for item in items.get("items"):
		...
		item["include_raw_materials_from_performa_invoice"] = items.get(
			"include_raw_materials_from_performa_invoice"
		)

	items.update({"company": company, "performa_invoice": performa_invoice})
...
def make_inter_company_purchase_order(source_name, target_doc=None):
	...
	return make_inter_company_transaction("Performa Invoice", source_name, target_doc)
...
def create_pick_list(source_name, target_doc=None):
	...
	doc = get_mapped_doc(
		...
		source_name,
		{
			"Performa Invoice": {"doctype": "Pick List", "validation": {"docstatus": ["=", 1]}},
			"Performa Invoice Item": {
				"doctype": "Pick List Item",
				"field_map": {"parent": "performa_invoice", "name": "performa_invoice_item"},
...
def update_produced_qty_in_so_item(performa_invoice, performa_invoice_item):
	# for multiple work orders against same Performa Invoice item
	linked_wo_with_so_item = frappe.db.get_all(
		"Work Order",
		["produced_qty"],
		{"performa_invoice_item": performa_invoice_item, "performa_invoice": performa_invoice, "docstatus": 1},
	)
	...
	frappe.db.set_value("Performa Invoice Item", performa_invoice_item, "produced_qty", total_produced_qty)
