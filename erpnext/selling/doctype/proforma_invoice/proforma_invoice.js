...
// frappe.ui.form.on('Performa Invoice', {
// 	refresh(frm) {
...
frappe.ui.form.on("Performa Invoice", {
	...
	refresh: function(frm) {
		...
		// frm.call({
		// 	method:'get_list_of_items_onload',//function name defined in python
		...
		if(frm.doc.docstatus === 1 && frm.doc.status !== 'Closed'
			...
			frm.add_custom_button(__('Update Items'), () => {
				erpnext.utils.update_child_items({
					...
					frm: frm,
					child_docname: "items",
					child_doctype: "Performa Invoice Detail",
	...
	get_items_from_internal_purchase_order(frm) {
		frm.add_custom_button(__('Purchase Order'), () => {
			erpnext.utils.map_current_doc({
				method: 'erpnext.buying.doctype.purchase_order.purchase_order.make_inter_company_performa_invoice',
	...
	onload: function(frm) {
		...
		// On cancel and amending a Performa Invoice with advance payment, reset advance paid amount
		if (frm.is_new()) {
...
frappe.ui.form.on("Performa Invoice Item", {
...
erpnext.selling.SalesOrderController = class SalesOrderController extends erpnext.selling.SellingController {
	...
	refresh(doc, dt, dn) {
			...
			if(this.frm.has_perm("submit")) {
				if(doc.status === 'On Hold') {
				   ...
				   if(flt(doc.per_delivered, 6) < 100 || flt(doc.per_billed) < 100) {
					   // close
					   this.frm.add_custom_button(__('Close'), () => this.close_performa_invoice(), __("Status"))
				   }
				}
			...
			if(doc.status !== 'Closed') {
				if(doc.status !== 'On Hold') {
					...
					if (this.frm.has_perm("submit")) {
						if(flt(doc.per_delivered, 6) < 100 || flt(doc.per_billed) < 100) {
							// hold
							this.frm.add_custom_button(__('Hold'), () => this.hold_performa_invoice(), __("Status"))
							// close
							this.frm.add_custom_button(__('Close'), () => this.close_performa_invoice(), __("Status"))
						}
					}
		...
		if (this.frm.doc.docstatus===0) {
			this.frm.add_custom_button(__('Quotation'),
				function() {
					erpnext.utils.map_current_doc({
						method: "erpnext.selling.doctype.quotation.quotation.make_performa_invoice",
	...
	create_pick_list() {
		frappe.model.open_mapped_doc({
			method: "erpnext.selling.doctype.performa_invoice.performa_invoice.create_pick_list",
			frm: this.frm
		})
	}
				...
				if(!r.message) {
					...
					const fields = [{
						...
						fields: [{
							...
							fieldtype: 'Data',
							fieldname: 'performa_invoice_item',
							reqd: 1,
							label: __('Performa Invoice Item'),
							hidden: 1
						}],
						data: r.message,
						get_data: () => {
						...
						primary_action: function() {
							...
							me.frm.call({
								...
								args: {
									items: data,
									company: me.frm.doc.company,
									performa_invoice: me.frm.docname,
	...
	make_material_request() {
		frappe.model.open_mapped_doc({
			method: "erpnext.selling.doctype.performa_invoice.performa_invoice.make_material_request",
			frm: this.frm
		})
	}
	...
	make_raw_material_request_dialog(r) {
		...
		var d = new frappe.ui.Dialog({
			...
			primary_action: function() {
				...
				me.frm.call({
					method: 'erpnext.selling.doctype.performa_invoice.performa_invoice.make_raw_material_request',
					args: {
						items: data,
						company: me.frm.doc.company,
						performa_invoice: me.frm.docname,
	...
	make_delivery_note(delivery_dates) {
		frappe.model.open_mapped_doc({
			method: "erpnext.selling.doctype.performa_invoice.performa_invoice.make_delivery_note",
			frm: this.frm,
			args: {
				delivery_dates
			}
	...
	make_sales_invoice() {
		frappe.model.open_mapped_doc({
			method: "erpnext.selling.doctype.performa_invoice.performa_invoice.make_sales_invoice",
			frm: this.frm
		})
	}
	...
	make_maintenance_schedule() {
		frappe.model.open_mapped_doc({
			method: "erpnext.selling.doctype.performa_invoice.performa_invoice.make_maintenance_schedule",
			frm: this.frm
		})
	}
	...
	make_project() {
		frappe.model.open_mapped_doc({
			method: "erpnext.selling.doctype.performa_invoice.performa_invoice.make_project",
			frm: this.frm
		})
	}
	...
	make_inter_company_order() {
		frappe.model.open_mapped_doc({
			method: "erpnext.selling.doctype.performa_invoice.performa_invoice.make_inter_company_purchase_order",
			frm: this.frm
		});
	}
	...
	make_maintenance_visit() {
		frappe.model.open_mapped_doc({
			method: "erpnext.selling.doctype.performa_invoice.performa_invoice.make_maintenance_visit",
			frm: this.frm
		})
	}
	...
	make_purchase_order(){
		...
		if(!pending_items){
			frappe.throw({message: __("Purchase Order already created for all Performa Invoice items"), title: __("Note")});
		}
		...
		var dialog = new frappe.ui.Dialog({
			...
			primary_action (args) {
				...
				return frappe.call({
					method: "erpnext.selling.doctype.performa_invoice.performa_invoice." + method,
					freeze: true,
						...
						if(!r.exc) {
							if (!args.against_default_supplier) {
								...
								frappe.route_options = {
									"performa_invoice": me.frm.doc.name
								}
	...
	update_status(label, status){
		...
		frappe.call({
			method: "erpnext.selling.doctype.performa_invoice.performa_invoice.update_status",
