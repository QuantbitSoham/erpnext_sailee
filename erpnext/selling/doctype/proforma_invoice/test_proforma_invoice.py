...
class TestProformaInvoice(FrappeTestCase):
	...
	def test_make_material_request(self):
		so = make_proforma_invoice(do_not_submit=True)
	...
	def test_so_billed_amount_against_return_entry(self):
		...
		si1.update_billed_amount_in_proforma_invoice = 1
		si1.submit()
		so.load_from_db()
	...
	def test_return_against_proforma_invoice(self):
		so = make_proforma_invoice()
		...
		dn1.items[0].against_proforma_invoice = so.name
		...
		si1.items[0].proforma_invoice = so.name
	...
	def test_reserved_qty_for_partial_delivery_with_packing_list(self):
		...
		so = make_proforma_invoice(item_code="_Test Product Bundle Item")
	...
	def test_proforma_invoice_on_hold(self):
		so = make_proforma_invoice(item_code="_Test Product Bundle Item")
	...
	def test_update_child_adding_new_item(self):
		so = make_proforma_invoice(item_code="_Test Item", qty=4)
		...
		update_child_qty_rate("Performa Invoice", trans_item, so.name)
	...
	def test_update_child_removing_item(self):
		so = make_proforma_invoice(**{"item_list": [{"item_code": "_Test Item", "qty": 5, "rate": 1000}]})
		...
		self.assertRaises(
			frappe.ValidationError, update_child_qty_rate, "Performa Invoice", trans_item, so.name
		)
	...
	def test_update_child_with_precision(self):
		...
		precision = get_field_precision(frappe.get_meta("Performa Invoice Item").get_field("rate"))

		make_property_setter("Performa Invoice Item", "rate", "precision", 7, "Currency")
		so = make_proforma_invoice(item_code="_Test Item", qty=4, rate=200.34664)
		...
		make_property_setter("Performa Invoice Item", "rate", "precision", precision, "Currency")
	...
	def test_update_child_qty_rate_with_workflow(self):
		...
		workflow = make_proforma_invoice_workflow()
		so = make_proforma_invoice(item_code="_Test Item", qty=1, rate=150, do_not_submit=1)
	...
	def test_update_child_product_bundle(self):
		...
		so = make_proforma_invoice(item_code="_Test Item", warehouse=None)
		...
		update_child_qty_rate("Performa Invoice", added_item, so.name)
		...
		update_child_qty_rate("Performa Invoice", update_uom_conv_factor, so.name)
	...
	def test_update_child_with_tax_template(self):
		...
		so = make_proforma_invoice(item_code=item, qty=1, do_not_save=1)
		...
		update_child_qty_rate("Performa Invoice", items, so.name)
	...
	def test_block_delivery_note_against_cancelled_proforma_invoice(self):
		so = make_proforma_invoice()
	...
	def test_service_type_product_bundle(self):
		...
		so = make_proforma_invoice(item_code="_Test Service Product Bundle", warehouse=None)
	...
	def test_mix_type_product_bundle(self):
		...
		self.assertRaises(
			WarehouseRequired, make_proforma_invoice, item_code="_Test Mix Product Bundle", warehouse=""
		)
	...
	def test_drop_shipping(self):
		...
		from erpnext.selling.doctype.proforma_invoice.proforma_invoice import update_status as so_update_status
		...
		# create so, po and dn
		so = make_proforma_invoice(item_list=so_items, do_not_submit=True)
		so.submit()
		...
		self.assertEqual(po.items[0].proforma_invoice, so.name)
		...
		self.assertEqual(flt(frappe.db.get_value("Performa Invoice", so.name, "per_delivered"), 2), 100.00)
		po.load_from_db()
	...
	def test_drop_shipping_partial_order(self):
		...
		self.assertEqual(po1.items[0].proforma_invoice, so.name)
	...
	def test_product_bundles_in_so_are_replaced_with_bundle_items_in_po(self):
		"""
		Tests if the the Product Bundles in the Items table of Performa Invoices are replaced with
		their child items(from the Packed Items table) on creating a Purchase Order from it.
		"""
		from erpnext.selling.doctype.proforma_invoice.proforma_invoice import make_purchase_order
		...
		so = make_proforma_invoice(item_list=so_items)
	...
	def test_reserved_qty_for_closing_so(self):
		...
		so = make_proforma_invoice(item_code="_Test Item", qty=1)
	...
	def test_create_so_with_margin(self):
		so = make_proforma_invoice(item_code="_Test Item", qty=1, do_not_submit=True)
	...
	def test_terms_auto_added(self):
		so = make_proforma_invoice(do_not_save=1)
	...
	def test_terms_copied(self):
		so = make_proforma_invoice(do_not_copy=1, do_not_save=1)
	...
	def test_make_work_order(self):
			...
			po_items.append(
				{
					...
					"proforma_invoice_item": item.get("proforma_invoice_item"),
					"bom": item.get("bom"),
					"description": item.get("description"),
				}
			)
			so_item_name[item.get("proforma_invoice_item")] = item.get("pending_qty")
		...
		for item in so_item_name:
			wo_qty = frappe.db.sql(
				"select sum(qty) from `tabWork Order` where proforma_invoice=%s and proforma_invoice_item=%s",
				(so.name, item),
			)
	...
	def test_serial_no_based_delivery(self):
		...
		work_order.proforma_invoice = so.name
		work_order.submit()
		...
		serial_no_so = frappe.get_value("Serial No", reserved_serial_no, "proforma_invoice")
		...
		se.load_from_db()
		se.cancel()
		self.assertFalse(frappe.db.exists("Serial No", {"proforma_invoice": so.name}))

	def test_advance_payment_entry_unlink_against_proforma_invoice(self):
		...
		pe = get_payment_entry("Performa Invoice", so.name, bank_account="_Test Bank - _TC")
		pe.reference_no = "1"
		...
		so_doc = frappe.get_doc("Performa Invoice", so.name)
	...
	def test_cancel_proforma_invoice_after_cancel_payment_entry(self):
		from erpnext.accounts.doctype.payment_entry.test_payment_entry import get_payment_entry

		# make a Performa Invoice
		so = make_proforma_invoice()
		...
		# create a payment entry against Performa Invoice
		pe = get_payment_entry("Performa Invoice", so.name, bank_account="_Test Bank - _TC")
		pe.reference_no = "1"
		pe.reference_date = nowdate()
		pe.paid_from_account_currency = so.currency
		...
		# Cancel Performa Invoice
		try:
			so_doc = frappe.get_doc("Performa Invoice", so.name)
			so_doc.cancel()
		except Exception:
			self.fail("Can not cancel Performa Invoice with linked cancelled payment entry")

	def test_work_order_pop_up_from_proforma_invoice(self):
		"Test `get_work_order_items` in Performa Invoice picks the right BOM for items to manufacture."
	...
	def test_request_for_raw_materials(self):
		...
		so = make_proforma_invoice(**{"item_list": [{"item_code": item.item_code, "qty": 1, "rate": 1000}]})
		so.submit()
		...
		self.assertEqual(mr_doc.items[0].proforma_invoice, so.name)

	def test_so_optional_blanket_order(self):
		"""
		Expected result: Blanket order Ordered Quantity should only be affected on Performa Invoice with against_blanket_order = 1.
		Second Performa Invoice should not add on to Blanket Orders Ordered Quantity.
		"""

		bo = make_blanket_order(blanket_order_type="Selling", quantity=10, rate=10)

		so = make_proforma_invoice(item_code="_Test Item", qty=5, against_blanket_order=1)
		so_doc = frappe.get_doc("Performa Invoice", so.get("name"))
		# To test if the SO has a Blanket Order
		self.assertTrue(so_doc.items[0].blanket_order)

		so = make_proforma_invoice(item_code="_Test Item", qty=5, against_blanket_order=0)
		so_doc = frappe.get_doc("Performa Invoice", so.get("name"))
	...
	def test_so_cancellation_when_si_drafted(self):
		"""
		Test to check if Performa Invoice gets cancelled if Sales Invoice is in Draft state
		Expected result: Performa Invoice should not get cancelled
		"""
		so = make_proforma_invoice()
		so.submit()
		si = make_sales_invoice(so.name)
	...
	def test_so_cancellation_after_si_submission(self):
		"""
		Test to check if Performa Invoice gets cancelled when linked Sales Invoice has been Submitted
		Expected result: Performa Invoice should not get cancelled
		"""
		so = make_proforma_invoice()
		so.submit()
	...
	def test_so_cancellation_after_dn_submission(self):
		"""
		Test to check if Performa Invoice gets cancelled when linked Delivery Note has been Submitted
		Expected result: Performa Invoice should not get cancelled
		"""
		so = make_proforma_invoice()
		so.submit()
	...
	def test_so_cancellation_after_maintenance_schedule_submission(self):
		...
		ms.items[0].proforma_invoice = so.name
		ms.submit()
	...
	def test_so_cancellation_after_maintenance_visit_submission(self):
		...
		mv.purposes[0].prevdoc_doctype = "Performa Invoice"
	...
	def test_so_cancellation_after_work_order_submission(self):
		...
		so = make_proforma_invoice(item_code="_Test FG Item", qty=10)
		so.submit()
		make_wo_order_test_record(proforma_invoice=so.name)
	...
	def test_payment_terms_are_fetched_when_creating_sales_invoice(self):
		...
		so = make_proforma_invoice(uom="Nos", do_not_save=1)
		create_payment_terms_template()
		so.payment_terms_template = "Test Receivable Template"
		so.submit()

		si = create_sales_invoice(qty=10, do_not_save=1)
		si.items[0].proforma_invoice = so.name
	...
	def test_zero_amount_proforma_invoice_billing_status(self):
		from erpnext.accounts.doctype.sales_invoice.test_sales_invoice import create_sales_invoice
	...
	def test_so_billing_status_with_crnote_against_sales_return(self):
		...
		dn_ret.items[0].against_proforma_invoice = so.name
		...
		cr_note.items[0].proforma_invoice = so.name
		...
		cr_note.update_billed_amount_in_proforma_invoice = True
		cr_note.submit()
	...
	def test_so_back_updated_from_wo_via_mr(self):
		...
		so = make_proforma_invoice(item_list=[{"item_code": "_Test FG Item", "qty": 2, "rate": 100}])
		...
		self.assertEqual(wo.proforma_invoice, so.name)
		self.assertEqual(wo.proforma_invoice_item, so.items[0].name)
	...
	def test_proforma_invoice_with_shipping_rule(self):
		...
		proforma_invoice = make_proforma_invoice(do_not_save=True)
		proforma_invoice.shipping_rule = shipping_rule.name

		proforma_invoice.items[0].qty = 1
		proforma_invoice.save()
		self.assertEqual(proforma_invoice.taxes[0].tax_amount, 50)

		proforma_invoice.items[0].qty = 2
		proforma_invoice.save()
		self.assertEqual(proforma_invoice.taxes[0].tax_amount, 100)

		proforma_invoice.items[0].qty = 3
		proforma_invoice.save()
		self.assertEqual(proforma_invoice.taxes[0].tax_amount, 200)

		proforma_invoice.items[0].qty = 21
		proforma_invoice.save()
		self.assertEqual(proforma_invoice.taxes[0].tax_amount, 0)

	def test_proforma_invoice_partial_advance_payment(self):
		from erpnext.accounts.doctype.payment_entry.test_payment_entry import (
		...
		# Create a advance payment against that Performa Invoice
		pe = get_payment_entry("Performa Invoice", so.name, bank_account="_Test Bank - _TC")
		pe.reference_no = "1"
		pe.reference_date = nowdate()
		pe.paid_from_account_currency = so.currency
...
def make_proforma_invoice(**args):
	so = frappe.new_doc("Performa Invoice")
...
def make_proforma_invoice_workflow():
