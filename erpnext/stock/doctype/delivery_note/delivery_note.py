...
class DeliveryNote(SellingController):
	...
	def load_data(self):
		for i in self.get('items'):
			temp=i.against_sales_order
		so=frappe.db.get_list("Sales Order",fields=['name','payment_terms_template'],filters={'name':temp})
		for j in so:
			doc=frappe.get_doc("Sales Order",j.name)
			for k in doc.get('payment_schedule'):
				self.payment_terms_template=j.payment_terms_template
				self.append("payment_schedule",{
							"payment_term":k.payment_term,
							"description":k.description,
							"due_date":k.due_date,
							"mode_of_payment":k.mode_of_payment,
							"invoice_portion":k.invoice_portion,
							"discount_type":k.discount_type,
							"discount_date":k.discount_date,
							"discount":k.discount,
							"payment_amount":k.payment_amount,
							"outstanding":k.outstanding,
							"paid_amount":k.paid_amount,
							"discounted_amount":k.discounted_amount,
							"base_payment_amount":k.base_payment_amount,
							}
						)
	
	@frappe.whitelist()
	def load_dataself(self):
		temp=self.payment_terms_template
		ptt=frappe.db.get_list("Payment Terms Template",fields=['name'],filters={'name':temp})
		for j in ptt:
			doc=frappe.get_doc("Payment Terms Template",j.name)
			for k in doc.get('terms'):
				self.append("payment_schedule",{
							"payment_term":k.payment_term,
							"description":k.description,
							# "due_date":k.due_date,
	...
	# @frappe.whitelist()
	# def list_all_Batches(self,doctype,item,against_sales_order=None,filters=None):
	# 	# data = json.loads(item)
	# 	# frappe.msgprint(str(item))
	# 	# frappe.msgprint(against_sales_order)
	# 	query = """SELECT 
	# 					tabBatch.batch_id AS "BatchID", 
	# 					tabBatch.item AS "ItemCode", 
	# 				sum( tabStockLedger.actual_qty) AS "ActualQty", 
	# 					tabBatch.reference_doctype AS "ReferenceDoctype", 
	# 					tabBatch.reference_name AS "ReferenceName", 
	# 					tabStockLedger.warehouse AS "Warehouse", 
	# 					case when IFNULL(tabReceiptItem.base_rate, 0) = 0 then (IFNULL(tabInvoiceItem.base_rate, 0)) else  (IFNULL(tabReceiptItem.base_rate, 0)) end  AS "PurchaseRate",
	# 					case when IFNULL(tabReceiptItem.base_rate, 0) = 0 then tabInvoice.supplier_name else  tabReceipt.supplier_name end  AS "supplier",
	# 					case when IFNULL(tabReceiptItem.base_rate, 0) = 0 then tabInvoice.name else  tabReceipt.name end  AS "PurchaseNo",
    #   					case when IFNULL(tabReceiptItem.base_rate, 0) = 0 then tabInvoice.posting_date else  tabReceipt.posting_date end  AS "PurchaseDate",					
    #   					tabBatch.stock_uom,tabBatch.item_name,tabBatch.batch_qty,tabItem.description,tabItem.gst_hsn_code,
	# 					ifnull(tabItemPrice.price_list_rate,0) price_list_rate,
	# 					tabsalesorder.parent salesorderagainst,tabsalesorder.name salesorderagainstitem,tabsalesorder.customer_item_code salesordercustitem,tabitemtax.item_tax_template itemtax
	# 				FROM `tabItem`
	# 				left join `tabBatch` on tabBatch.item = tabItem.item_code
	# 				LEFT JOIN `tabStock Ledger Entry` AS tabStockLedger ON (tabBatch.name = tabStockLedger.batch_no)
	# 				left join `tabPurchase Receipt Item` as tabReceiptItem ON (tabBatch.name = tabReceiptItem.batch_no and  tabBatch.item=tabReceiptItem.item_code)
	# 				left join `tabPurchase Invoice Item` as tabInvoiceItem ON (tabBatch.name = tabInvoiceItem.batch_no and  tabBatch.item=tabInvoiceItem.item_code)
    #  				left join `tabPurchase Receipt` as tabReceipt ON tabReceipt.name =  tabReceiptItem.parent
	# 				left join `tabPurchase Invoice` as tabInvoice ON tabInvoice.name =  tabInvoiceItem.parent
	# 				left join `tabItem Price` as tabItemPrice on (tabItemPrice.selling =1 and  tabBatch.item=tabItemPrice.item_code )
    #  				left join `tabSales Order Item` as tabsalesorder on (tabsalesorder.parent= %(salesorder)s and tabsalesorder.item_code =tabItem.item_code )
	# 				left join `tabItem Tax` as tabitemtax on ( tabitemtax.parent = tabItem.item_code and  tabitemtax.parentfield = 'taxes' and tabitemtax.parenttype= 'Item' )
	# 				WHERE (tabStockLedger.posting_date <= curdate())
	# 				and tabBatch.item IN  %(items)s GROUP BY tabBatch.batch_id, tabBatch.item,tabBatch.reference_doctype, tabStockLedger.warehouse ,PurchaseRate
	# 				HAVING SUM(IFNULL(tabStockLedger.qty_after_transaction, 0)) > 0 and SUM(IFNULL(tabStockLedger.actual_qty, 0)) >0
	# 				ORDER BY tabItem.item_code, tabStockLedger.posting_date DESC;
	# 				"""

	# 	# Pass the array as a parameter using the `tuple` function
	# 	data = frappe.db.sql(query, {"items": tuple(item),"salesorder":against_sales_order},as_dict=1)
  
	# 	if data:
	# 		# frappe.msgprint(str(data))
	# 		for row in data:			
	# 			self.append("itemwise_batch_details",{
	# 							"batch":row.BatchID,
	# 							"item":row.ItemCode,
	# 							"warehouse":row.Warehouse,
	# 							"purchase_rate":row.PurchaseRate,
	# 							"balance_qty":row.ActualQty,
	...
	# 							"sales_order_ref":row.salesorderagainst,
    #    							"customer_item_code":row.salesordercustitem,
    #     						"so_detail":row.salesorderagainstitem,
    #     						"actual_batch_qty":row.ActualQty,
	# 							"tax":row.itemtax
	# 							}
	# 							)	
	# 	else :
	...
	# @frappe.whitelist()
	# def Batch_selector(self,doctype,item=None,filters=None):
	# 	# Get the parent document
	# 	# Get the source and target child tables
	# 	list_1 = []
				
	# 	for row in self.itemwise_batch_details:
	# 		if row.check_1:
	# 			list_1.append({
	# 					"item_code": str(row.item),
	# 					"qty": str(row.qty),
	# 					"batch_no": str(row.batch),
	# 					"rate": str(row.purchase_rate),
	# 					"warehouse":str(row.warehouse),
	# 					"item_name":str(row.item_name),
	# 					"description":str(row.description),
	# 					"gst_hsn_code":str(row.gst_hsn_code),
	# 					"stock_uom":str(row.stock_uom),
	# 					"price_list_rate":str(row.price_list_rate),
	# 					"sales_order_ref":str(row.sales_order_ref),
    #  					"customer_item_code":str(row.customer_item_code),
    #                     "so_detail":str(row.so_detail),
    #                     "actual_batch_qty":str(row.actual_batch_qty),
    #                     "tax":str(row.tax),
	# 				})
	# 	for row in list_1:
	# 		self.append("items", {
    ...
    #              "amount":float(row["price_list_rate"])*float(row["qty"]),
	# 			 "against_sales_order": row["sales_order_ref"],
	# 			 "customer_item_code":row["customer_item_code"],
	# 			 "so_detail":row[ "so_detail"],
	# 			"actual_batch_qty":row["actual_batch_qty"],
    # 			"item_tax_template":row["tax"],
