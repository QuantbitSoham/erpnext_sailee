...
class PaymentReconciliation(Document):
	...
	# @frappe.whitelist()
	# def somsg(self):
	# 	for i in self.get('invoices'):
	# 		doc = frappe.db.get_list('', filters={'name': i.invoice_number},fields=["name", "bill_no"])
	# 		for s in doc:
	...
	def add_invoice_entries(self, non_reconciled_invoices):
		...
		for entry in non_reconciled_invoices:
			...
			inv.bill_no = entry.get("bill_no")
			inv.amount = flt(entry.get("invoice_amount"))
			inv.currency = entry.get("currency")
			inv.outstanding_amount = flt(entry.get("outstanding_amount"))
			if(self.party_type=="Supplier"):
				doc = frappe.get_doc("Purchase Invoice",inv.invoice_number)
				inv.supplier_invoice_no = doc.bill_no
				inv.supplier_invoice_date = doc.bill_date
