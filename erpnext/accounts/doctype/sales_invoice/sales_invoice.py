...
class SalesInvoice(SellingController):
	...
	def press(self):
		...
		for row in self.items:
			name = (row.item_code,row.rate,row.item_name,row.description,row.uom,row.item_tax_template,row.income_account,row.cost_center,row.expense_account,row.warehouse,row.quality_inspection,row.batch_no,row.item_group,row.gst_hsn_code)
			if name in by_name: 
				by_name[name]["qty"] += int(row.qty)
				by_name[name]["amount"] += int(row.amount)
				by_name[name]["total_amount"] += int(row.total_amount)
			else:
				by_name[name] = {
					"qty": row.qty,
					"amount": row.amount,
		...
		# frappe.msgprint(str(by_name))
		
		for (item_code,rate,item_name,description,uom,item_tax_template,income_account,cost_center,expense_account,warehouse,quality_inspection,batch_no,item_group,gst_hsn_code), data in by_name.items():
			self.append("sample_items",{
				"item_code": item_code,
				"item_name": item_name,
